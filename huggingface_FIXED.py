...
class HuggingFaceBackboneAdapter:
    ...
    def forward(self, inputs: Dict[str, Any]) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Model output (empty for Qwen3-TTS as hooks capture activations)
        """
        with torch.no_grad():
            if self._model_type == "qwen3_tts":
    ...
    def resolve_layer(self, alias: str) -> Optional[torch.nn.Module]:
        ...
        Args:
            alias: Layer name or alias (e.g., "text_encoder_out", "decoder_block_04")
            
        Returns:
            Optional[torch.nn.Module]: Resolved module or None if not found
        """
        if self._model_type != "qwen3_tts":
